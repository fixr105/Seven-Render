...
> This file mirrors the repository root [README.md](../README.md) for browsing under `docs/`.
...
Data access to Airtable is **via n8n** (`backend/src/services/airtable/n8nClient.ts`, `webhookConfig.ts`): GET webhooks for reads (with in-memory cache invalidated after writes), POST webhooks for creates/updates. See [system-overview.md](./system-overview.md) for module M1–M7 narrative and webhook-oriented field notes.
...
- [system-overview.md](./system-overview.md) — modules M1–M7, roles, Airtable/n8n tables and webhook naming  
