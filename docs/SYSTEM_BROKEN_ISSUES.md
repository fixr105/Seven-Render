...
- **E2E / local without matching n8n data:** Login and settings require real User Accounts in Airtable. If the logged-in user’s id is missing or inactive, `PATCH` settings can 404. Use the accounts in `TEST_USERS.md`.

**Optional improvement:**

- Document that Settings are not asserted in E2E when User Accounts / n8n are incomplete.
...
- Or add a separate, higher limit (or skip) for E2E via env.
...
**Note:** E2E hits real n8n for User Accounts, ledger, clients, loan applications, notifications, etc. Use the real test users in `TEST_USERS.md`.
...
3. **Settings in E2E:** Assert only when the logged-in user exists in User Accounts; otherwise expect 404 and skip.
