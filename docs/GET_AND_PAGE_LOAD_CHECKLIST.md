...
There must be no `setInterval`, cron, or polling that triggers n8n GET/POST except intentional B2C KAM sync (paused when tab hidden). All webhook calls occur when: (1) a page loads (including SPA route mount) and its `useEffect` runs, (2) the user explicitly triggers an action (e.g. Refresh with `forceRefresh=true`, Generate, Submit), or (3) POST mutations invalidate cache. Backend GETs use cache by default; `?forceRefresh=true` bypasses read cache (and still write-through).

### n8n duplicate workflows (ops)

If Executions still multiply after app-side caching: in n8n Cloud, deactivate older/standalone workflows that share the same webhook path as the production flow (e.g. `loanapplication`, `loanproducts`). Keep a **single** active workflow set per path. Duplicate active listeners run one execution per workflow per HTTP call.
