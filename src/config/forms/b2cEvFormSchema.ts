...
const supportPersonFields = (prefix: 'coApplicant' | 'guarantor', relationOptions: B2cEvFieldOption[]): B2cEvFieldDef[] => [
  ...
  { key: `${prefix}.drivingLicense`, label: 'Driving License', type: 'text', required: prefix === 'guarantor' },
...
export const B2C_EV_STAGES: B2cEvStage[] = [
  ...
  },
  {
    ...
    fields: [
      { key: 'loan.vehiclePrice', label: 'Vehicle Price (₹)', type: 'currency', required: true },
      { key: 'loan.gstRate', label: 'GST Rate', type: 'select', required: true },
      { key: 'loan.insurance', label: 'Insurance (₹)', type: 'currency', required: true },
      { key: 'loan.registration', label: 'Registration (₹)', type: 'currency', required: true },
      { key: 'loan.accessories', label: 'Accessories (₹)', type: 'currency', required: true },
      { key: 'loan.calculator.customerPayment', label: 'Payment from Customer (₹)', type: 'currency', required: true },
      { key: 'loan.taxInvoiceValue', label: 'Tax Invoice Value (₹)', type: 'currency', readOnly: true },
