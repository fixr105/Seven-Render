...
describe('LoanController - P0 Tests', () => {
  ...
  describe('updateClientApplicationStatus', () => {
    const applicableDraftAndKam = JSON.stringify([
      { key: LoanStatus.DRAFT, label: 'Draft', order: 10 },
      { key: LoanStatus.UNDER_KAM_REVIEW, label: 'Under KAM Review', order: 20 },
    ]);

    const mockLoanProduct = {
      id: 'prod-rec',
      'Product ID': 'PROD_DRAFT_KAM',
      'Applicable Statuses': applicableDraftAndKam,
    };

    const clientAppDraft = {
      ...mockLoanApplications[0],
      id: 'rec-client-status',
      'Client': 'CLIENT001',
      'Loan Product': 'PROD_DRAFT_KAM',
      'Status': LoanStatus.DRAFT,
    };

    it('calls postLoanApplication with normalized Status for an allowed client transition', async () => {
      const clientUser: AuthUser = {
        id: 'user-client',
      ...
      (mockN8nClientInstance.fetchTable as jest.Mock).mockImplementation(async (tableName: string) => {
        if (tableName === 'Loan Application') return [clientAppDraft];
        if (tableName === 'Loan Products') return [mockLoanProduct];
        return [];
      });

      mockRequest = {
        user: clientUser,
        params: { id: 'rec-client-status' },
        body: { status: 'under_kam_review', notes: 'Moving forward' },
      };

      await controller.updateClientApplicationStatus(mockRequest as Request, mockResponse as Response);

      expect(mockResponse.json).toHaveBeenCalledWith(
        expect.objectContaining({ success: true, message: 'Status updated successfully' })
      );
      expect(mockN8nClientInstance.postLoanApplication).toHaveBeenCalled();
      const posted = mockN8nClientInstance.getPostedData('Loan Applications');
      expect(posted.length).toBeGreaterThan(0);
      expect(posted[posted.length - 1].Status).toBe(LoanStatus.UNDER_KAM_REVIEW);
    });

    it('returns 404 when client does not own the application', async () => {
      const clientUser: AuthUser = {
        id: 'user-other',
      ...
      mockRequest = {
        user: clientUser,
        params: { id: 'rec-client-status' },
        body: { status: LoanStatus.UNDER_KAM_REVIEW },
      };
    ...
    it('returns 400 when status is not configured on the loan product', async () => {
      const clientUser: AuthUser = {
        id: 'user-client',
        email: 'client@test.com',
        role: UserRole.CLIENT,
      ...
      mockRequest = {
        user: clientUser,
        params: { id: 'rec-client-status' },
        body: { status: LoanStatus.DISBURSED },
      };

      await controller.updateClientApplicationStatus(mockRequest as Request, mockResponse as Response);

      expect(mockResponse.status).toHaveBeenCalledWith(400);
      const payload = (mockResponse.json as jest.Mock).mock.calls[0][0] as { error?: string };
