...
export class LedgerController {
  ...
  async getClientLedger(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        allLedgerEntries = await n8nClient.fetchTable('Commission Ledger', useCache);
      } catch (error: any) {
  ...
  async createLedgerQuery(req: Request, res: Response): Promise<void> {
    try {
      ...
      const ledgerEntries = await n8nClient.fetchTable('Commission Ledger'); // Disable cache
  ...
  async getKAMLedger(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!managedClientIds.some((id) => matchIds(id, clientId as string))) {
